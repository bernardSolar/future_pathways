    ...
    def create_3d_figure(self):
        ...
        for path_name, path_config in self.paths.items():
            if path_name == 'Historical Trajectory':
                ...
                # Axes: x = Material Use, y = CO₂e Emissions, z = Growth
                malm_materials = np.array([106, 110, 115, 120, 100, 90, 85, 80, 80])
                malm_emissions = np.array([50, 55, 65, 65, 30, 0, -10, -15, -15])
                malm_growth   = np.array([2.5, 2.0, 1.8, 1.5, 0.5, 0.0, 0.5, 1.0, 1.0])

                # Add continuous line trace for the Malm scenario with a dashed line style
                fig.add_trace(go.Scatter3d(
                    x=malm_materials,
                    y=malm_emissions,
                    z=malm_growth,
                ...
                # Calculate future coordinates for other pathways
        ...
        # Set up the layout; adjust title if needed to reflect extended timelines
        fig.update_layout(
            scene=dict(
                ...
                yaxis=dict(range=[-20, 80]),  # Extended CO₂e axis range to 80 Gt
